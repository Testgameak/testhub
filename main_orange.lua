...
if getgenv().OrangeHub_Loaded then
   warn("[Orange Hub] Already loaded!")
   return
end
getgenv().OrangeHub_Loaded = true
do
...
- ORANGE HUB (Blox Fruit) — Language System V2.1 By Mio Herruto
]]
...
_G.LangCode = GetSetting('Language_Save', 'en')
_G.LangData = _G.AllLangs[_G.LangCode] or _G.AllLangs.en
...
function ApplyLanguage(code)
    ...
    pcall(function()
        Library:Notify({Title = 'Orange Hub', Message = T('common.langApplied'), Duration = 3})
    end)
end
...
local _NotifyContainer = nil
    ...
    function Library:Notify(Args)
        ...
        Library:Create("UIStroke", { Parent = Card, Color = Color3.fromRGB(100, 55, 0), Thickness = 1 })
        ...
        local Progress = Library:Create("Frame", {
            ...
            BackgroundColor3 = Color3.fromRGB(100, 55, 0),
            BorderSizePixel = 0,
...
local WelcomeCard = Library:Create("Frame", {
    ...
    Library:Create("TextLabel", {
        ...
        Font = Enum.Font.GothamBold,
        RichText = true,
        Text = "Welcome to ORANGE HUB CAT,  <font color='#FF8C00'>" .. PlayerName .. "!</font>",
        TextColor3 = Color3.fromRGB(255, 255, 255),
        TextSize = 13,
        TextXAlignment = Enum.TextXAlignment.Left,
        LayoutOrder = 1,
    ...
    Library:Create("TextLabel", {
        ...
        Text = "Orange Hub | Join Discord For Updates!",
...
function Page:InfoNguyenNhat(InfoArgs)
    ...
    local THEME_DIM = Color3.fromRGB(100, 55, 0)
            ...
            do
                ...
                function Setting:AddList(Name)
                    ...
                    local function OnValue(value)
                        Title_2.TextColor3 = value and Color3.fromRGB(255, 140, 0) or Color3.fromRGB(255, 255, 255)
...
local LibWindow = Library:Window({
    Title = 'ORANGE HUB | Vietnamese <3',
    SubTitle = 'By Nguyễn Minh Nhật >_< | Version: 1.6',
})

local Status = LibWindow:NewPage({
    Title = 'Info & Server',
    Desc = 'Player Stats & Server Information',
    Icon = 7040410130,
})

local Farm = LibWindow:NewPage({
    Title = 'Tab Farming',
    Desc = 'Auto Level, Quests & Boss Farming',
    Icon = 6034418511,
})

local Others = LibWindow:NewPage({
    Title = 'Stack Farm',
    Desc = 'Combine Multiple Farming Tasks',
    Icon = 6022668911,
})

local Maestry = LibWindow:NewPage({
    Title = 'Farm Mastery',
    Desc = 'Level Up Weapons & Devil Fruit Mastery',
    Icon = 6023426941,
})

local Event = LibWindow:NewPage({
    Title = 'Sea Event',
    Desc = 'Auto Hunt Sea Beasts & Ocean Events',
    Icon = 6023426906,
})

local SeaStack = LibWindow:NewPage({
    Title = 'Sea Stack',
    Desc = 'Advanced Sea Event Combinations',
    Icon = 6023426906,
})

local SeaSettings = LibWindow:NewPage({
    Title = 'Sea Settings',
    Desc = 'Configure Sea Event Automation',
    Icon = 6023426906,
})

local Race = LibWindow:NewPage({
    Title = 'Upgrade V4',
    Desc = 'Race Awakening, Trials & Gears',
    Icon = 6031260782,
})

local Dojo = LibWindow:NewPage({
    Title = 'Dojo & Drago Race',
    Desc = 'Dojo Tasks & Dragon Race Features',
    Icon = 6035145359,
})

local Get = LibWindow:NewPage({
    Title = 'Get Item & Upgrade',
    Desc = 'Auto Obtain Swords, Accessories & Materials',
    Icon = 6031084743,
})

local Fruit = LibWindow:NewPage({
    Title = 'Raid & Fruit',
    Desc = 'Auto Raids, Sniper & Devil Fruit Management',
    Icon = 6031260782,
})

local Player = LibWindow:NewPage({
    Title = 'Local Player',
    Desc = 'Character Modifications & Movement (Noclip/Speed)',
    Icon = 6034287513,
})

local Shop = LibWindow:NewPage({
    Title = 'Local Shop',
    Desc = 'Quick Buy Fighting Styles, Haki & Items',
    Icon = 6022668909,
})

local Esp = LibWindow:NewPage({
    Title = 'Stats & ESP',
    Desc = 'Auto Allocate Stats & Visual Assistance',
    Icon = 11155851001,
})

local Teleport = LibWindow:NewPage({
    Title = 'Tab Teleport',
    Desc = 'Fast Travel Between Islands & Seas',
    Icon = 6034287513,
})

local Setting = LibWindow:NewPage({
    Title = 'Setting & UI',
    Desc = 'Script Configuration & Interface Tweaks',
    Icon = 6031280882,
})

local Visual = LibWindow:NewPage({
    Title = T('pages.visual_title'),
    Desc  = T('pages.visual_desc'),
    Icon  = 6034287513,
})
...
Status:Discord({
    ServerName = "ORANGE HUB",
    ServerDescription = "Join My Sever To Get New Update Orange Hub.",
            ...
            if G.Auto_Mink then
                function Notify(msg)
                    if msg ~= lastMsg_Mink then
                        lastMsg_Mink = msg
                        Library:Notify({ Title    = "Auto Mink (ORANGE HUB)", Message  = msg, Duration = 6 })
                    end
                end
            ...
            if G.Auto_Human then
                function Notify(msg)
                    if msg ~= lastMsg_Human then
                        lastMsg_Human = msg
                        Library:Notify({ Title    = "Auto Human (ORANGE HUB)", Message  = msg, Duration = 6 })
                    end
                end
        ...
        pcall(function()
            if G.Auto_Skypiea then
                function Notify(msg)
                    if msg ~= lastMsg then
                        lastMsg = msg
                        Library:Notify({ Title    = "Auto Skypiea (ORANGE HUB)", Message  = msg, Duration = 6 })
                    end
                end
            ...
            if G.Auto_Fish then
                function Notify(msg)
                    if msg ~= lastMsg_Fish then
                        lastMsg_Fish = msg
                        Library:Notify({ Title    = "Auto Fisch (ORANGE HUB)", Message  = msg, Duration = 6 })
                    end
                end
...
Setting:Button({
    ...
    Callback = function()
        if SaveSettings then
            SaveSettings()
            Library:Notify({Title = "Orange Hub", Message = T('common.saved'), Duration = 5})
        else
...
Setting:Button({
    ...
    Callback = function()
        if isfile and isfile(FullPath) then
            delfile(FullPath)
            _G.SaveData = {}
            Library:Notify({Title = "Orange Hub", Message = "Reset Config Done!", Duration = 5})
        else
            Library:Notify({Title = "Orange Hub", Message = "Reset Config Done!", Duration = 5})
        end
    end,
...
local Message = ""
pcall(function()
    Message = game:HttpGet("https://raw.githubusercontent.com/AlexHerrySeek/OrangeHub/refs/heads/main/Notifications")
end)
KhThy:NotiSea("<Color=Red>WELCOME TO<Color=/> <Color=Green>ORANGE HUB<Color=/> <Color=Yellow>!<Color=/>")
wait(5)
if Message ~= "" then
    task.wait(1)
